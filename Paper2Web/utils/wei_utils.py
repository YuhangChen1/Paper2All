...
def get_agent_config(model_type):
    agent_config = {}
    openai_compatible_config = ChatGPTConfig().as_dict()
    openai_compatible_config.pop('logit_bias', None)
    if model_type == 'qwen':
        agent_config = {
        ...
        agent_config = {
            ...
            "model_config": openai_compatible_config,
        ...
        agent_config = {
            'model_type': 'qwen/qwen-2.5-vl-72b-instruct',
            'model_platform': ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    elif model_type == 'openrouter_qwen_vl_7b':
        agent_config = {
            'model_type': 'qwen/qwen-2.5-vl-7b-instruct',
            'model_platform': ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    elif model_type == 'openrouter_qwen_7b':
        agent_config = {
            'model_type': 'qwen/qwen-2.5-7b-instruct',
            'model_platform': ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    elif model_type == 'openrouter_qwen3_14b':
        agent_config = {
            'model_type': 'qwen/qwen3-14b',
            'model_platform': ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    elif model_type == 'openrouter_qwen2_5_vl_32b':
        agent_config = {
            'model_type': 'qwen/qwen2.5-vl-32b-instruct:free',
            'model_platform': ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    elif model_type == 'openrouter_qwen3_30b_a3b':
        agent_config = {
            'model_type': 'qwen/qwen3-30b-a3b-instruct-2507',
            'model_platform': ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    elif model_type == 'openrouter_qwen3_coder':
        agent_config = {
            'model_type': 'qwen/qwen3-coder',
            'model_platform': ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    elif model_type == 'openrouter_qwen3_coder_free':
        agent_config = {
            'model_type': 'qwen/qwen3-coder:free',
            'model_platform': ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    elif model_type == 'openrouter_qwen2_5_VL_72B':
        agent_config = {
            'model_type': 'qwen/qwen2.5-vl-72b-instruct',
            'model_platform': ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    elif model_type == 'openrouter_qwen3_253B_a22b':
        agent_config = {
            'model_type': 'qwen/qwen3-235b-a22b-2507',
            'model_platform': ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    elif model_type == 'openrouter_o1':
        agent_config = {
            "model_type": "openai/o1",
            "model_platform": ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    elif model_type == 'openrouter_4o':
        agent_config = {
            "model_type": "openai/gpt-4o",
            "model_platform": ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    elif model_type == 'openrouter_4_1':
        agent_config = {
            "model_type": "openai/gpt-4.1-min",
            "model_platform": ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    elif model_type == 'openrouter_gemini_flash':
        agent_config = {
            "model_type": "google/gemini-2.5-flash",
            "model_platform": ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    elif model_type == 'openrouter_gemini_flash_lite':
        agent_config = {
            "model_type": "google/gemini-2.5-flash-lite",
            "model_platform": ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    elif model_type == 'glm-4.5':
        agent_config = {
            "model_type": "z-ai/glm-4.5v",
            "model_platform": ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    elif model_type == 'Llama-4':
        agent_config = {
            "model_type": "meta-llama/llama-4-maverick",
            "model_platform": ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    elif model_type == 'qwen_coder_best':
        agent_config = {
            "model_type": "qwen/qwen3-coder",
            "model_platform": ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        ...
        agent_config = {
            "model_type": "deepseek/deepseek-v3.2-exp",
            "model_platform": ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    elif model_type == 'grok':
        agent_config = {
            "model_type": "x-ai/grok-code-fast-1",
            "model_platform": ModelPlatformType.OPENROUTER,
            'model_config': OpenRouterConfig().as_dict(),
        }
    else:
...
def edit_modular(
    ...
    for attempt in range(num_retries + 1):
        ...
        if attempt < num_retries:
            print(f"Retrying... Attempt {attempt + 1} of {num_retries}")
            msg = error
            msg += '\nFix your code and try again. The website is a single-page pptx.'
            if prompt_type != 'initial':
                msg += '\nAssume that you have had a Presentation object named "website" and a slide named "slide".'
...
def gen_layout_parallel(agent, prompt, num_retries, existing_code='', slide_width=0, slide_height=0, tmp_name='tmp'):
    ...
    existing_code += f'''
website = create_website(width_inch={slide_width}, height_inch={slide_height})
slide = add_blank_slide(website)
save_presentation(website, file_name="website_{tmp_name}.pptx")
'''
    agent.reset()
    log = []
    cumulative_input_token, cumulative_output_token = 0, 0
...
def is_website_filled(
    ...
    along with the overall dimensions of the website and maximum allowed
    margins, this function determines whether the boxes collectively
    fill the website within those margin constraints.
    ...
    :param overall_width: Total width of the website
    :param overall_height: Total height of the website
    :param max_lr_margin: Maximum allowed left and right margins
    :param max_tb_margin: Maximum allowed top and bottom margins
    :return: True if the bounding boxes fill the website (with no big leftover spaces),
             False otherwise.
    """

    # If there are no bounding boxes, we consider the website unfilled.
...
def html_to_png(html_abs_path, website_width_default, website_height_default, output_path):
    html_file = html_abs_path

    try:
        w, h = asyncio.run(rendered_dims(html_file))
    except:
        w = website_width_default
        h = website_height_default
...
def get_snapshot_from_section(leaf_section, section_name, name_to_hierarchy, leaf_name, section_code, empty_website_path='website.pptx'):
    ...
    website = Presentation(empty_website_path)
    ...
    save_presentation(website, file_name=f"{hierarchy_overflow_name}.pptx")
    ppt_to_images(
        f"{hierarchy_overflow_name}.pptx",
        hierarchy_overflow_name,
        dpi=200
    )
    website_image_path = os.path.join(f"{hierarchy_overflow_name}", "slide_0001.jpg")
    website_image = Image.open(website_image_path)

    website_width = emu_to_inches(website.slide_width)
    website_height = emu_to_inches(website.slide_height)
